...
const Allusers = () => {
  ...
  const { data: users = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        "https://file-upload-server-gitfair.glitch.me/users"
      );
  ...
  const handleDeleteUser = (user) => {
    console.log(user);
    fetch(
      `https://git-fair-server-a8yyvsb4r-hopefyjamshed.vercel.app/user/${user?._id}`,
      {
        method: "DELETE",
        headers: {
  ...
  return (
    <div className="bg-white  lg:ml-10 lg:mr-10 ">
      <h2 className="text-3xl text-black mb-4  font-semibold">All Users</h2>
              ...
              <tr key={user._id}>
                ...
                <td>
                  <label
                    ...
                    className="btn btn-xs btn-danger bg-red-600  text-[15px] text-center rounded-md  uppercase"
                  >
                    Delete
                  </label>
                </td>
